...
def place_order(client, symbol, qty, price):
    try:
        order = client.place_order(symbol=symbol, category='linear', isLeverage='1', side="Buy", order_type="Limit", qty=qty, price=price)
...
def set_leverage(client, symbol, leverage):
    try:
        ...
        response = client.set_leverage(symbol=symbol, buyLeverage=leverage_string, sellLeverage=leverage_string, category= 'linear')
...
def custom_round(number, step_size):
    return int(number / step_size) * step_size
...
def calculate_order_quantity(client, symbol, total_balance, proportion_of_balance, avgPrice, currentPrice):
    ...
    basic_qty = custom_round(order_value / currentPrice, qty_step)
    ...
    if avgPrice > currentPrice:
        ...
        logging.info(f'Diff between ask price {currentPrice} and average buy price {avgPrice}: {price_percentage_difference}')
...
def main():
    ...
    profit_threshold = 3.00  # Define the profit threshold for closing positions
    leverage_level = 4  # Define the desired leverage level
    proportion_of_balance = 0.001  # 0.1% of total balance
    ...
    while True:
        ...
        for symbol in symbols:
            ...
            if position:
                ...
                logging.info(f'Symbol={symbol}, PNL={pnl}/absolute={pnl_absolute}, Position Value={position_value}, profit_threshold={profit_threshold}')
                ...
                elif pnl < 0: #only trade on negative PNL
                    logging.info(f'PNL Negative, sending order to catch up for symbol {symbol}')

                    # def calculate_order_quantity(client, symbol, total_balance, proportion_of_balance, avgPrice, currentPrice):
                    bid_price, _ = get_ticker_info(client, symbol)
                    order_qty = calculate_order_quantity(client, symbol, total_balance, proportion_of_balance, avg_price, bid_price)
